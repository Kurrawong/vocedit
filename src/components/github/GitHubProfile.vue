...
const githubUser = computed(() => {
  const user = snapshot.value.context.githubUser
  if (!user) {
...
const handleOpenChange = (open: boolean) => {
  if (!open) {
    send({ type: 'integration.github.auth.profile.close' })
  }
}
</script>
...
<template>
  <Dialog :open="isOpen" @update:open="handleOpenChange">
    <DialogContent>
      ...
      <DialogFooter>
        <Button variant="outline" @click="send({ type: 'integration.github.auth.profile.close' })">
          Close
        </Button>
      </DialogFooter>
    </DialogContent>
